...
$(document).ready(function(){
	...
	$("a#course").click(function(event){
            ...
            // top menue color changes when it is selected
		...
		// pop up create course box when 'Create Course' is clicked
            ...
            $('a.create-course-content').click(function() {
	...
	$('#create-course-content').click(function(event) {
            ...
            var file = document.getElementById('course-content').files[0];
            var fd = new FormData();
            fd.append('upload_file', file);
            fd.append('action', "add");
            fd.append('topic', $('#course-content').val());
            fd.append('description', $('#course-content-description').val());
            fd.append('courseid',$('#course-id').val());
            var xhr = new XMLHttpRequest();
	...
	$.ajax({
		...
		success: function(data){
                    ...
                    $(data).find("mycourse").each(function(){
                            ...
                            thtml = thtml + "<div id = 'my-courses-row'><div id = 'my-course-img'><img class = 'small-size-image' src= '" + img + "'/></div>";

                            thtml = thtml + '<div id = "my-course-text"><a href="course.php?action=list&title='+$(this).find("title").text()+'"><h3>' + $(this).find("title").text() + "<br/></h3></a>";
                            ...
                            thtml = thtml + "<a href=''>" + $(this).find("group_name").text() + "</a></div>"

                            thtml = thtml + "<div id = 'my-course-button'>Button</div>";
                            thtml = thtml + "</div>";
	...
	$.ajax({
                ...
                success: function(data){
                        ...
                        $(data).find("course").each(function(){
                                ...
                                var text = "<a href=''><h3>" + $(this).find("title").text() + "</h3></a>";
